...
class NodeFlavor extends Emitter {
  ...
  test(handler) {
    ...
    if ( typeof handler !== 'function' ) {
      this.state.ERROR = new TypeError('Test handler is not a function');
      return this;
    }
  ...
  forEachExpectation(fn) {
    if ( typeof fn !== 'function' ) throw new TypeError(`Expected a function as an argument, but instead received a type ${typeof fn}`);
    for ( let i = 0; i < this.expectations.length; ++i ) {
      const expect = this.expectations[i];
      fn(expect);
    }
